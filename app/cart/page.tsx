    ...
    <div className="min-h-screen bg-background text-foreground pb-20">
      ...
      {cart.length > 0 ? (
        <>
          <section className="px-4 py-6 space-y-4">
            {cart.map((item) => (
              <div key={item.variantId} className="bg-muted/50 rounded-2xl p-4">
                <div className="flex gap-4 mb-4">
                  <div className="w-24 h-24 flex-shrink-0 bg-background rounded-xl overflow-hidden">
                    <img 
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1 text-sm text-muted-foreground">
                      <p>
                        {t('size')}: <span className="font-medium text-foreground">{item.size}</span>
                      </p>
                      {item.color && (
                ...
                <div className="flex items-center justify-between">
                  <div className="inline-flex items-center gap-3 bg-background rounded-full px-4 py-2">
                    <button 
          ...
          <section className="px-4 pb-6">
            <div className="bg-muted/50 rounded-2xl p-6 space-y-4">
              <h3 className="font-serif text-xl font-bold text-foreground pb-3 border-b border-border">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <Currency value={subtotal} className="font-medium text-foreground" />
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>{t('shipping')}</span>
                  <Currency value={shipping} className="font-medium text-foreground" />
                </div>
                <div className="pt-3 border-t border-border flex justify-between">
                  <span className="font-serif text-lg font-bold text-foreground">{t('total')}</span>
                  <Currency value={total} className="font-serif text-xl font-bold text-foreground" />
                </div>
              </div>
      ...
      {showConfirmation && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center px-4">
          <div className="bg-background w-full max-w-md rounded-3xl p-8 text-center shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="flex flex-col gap-3">
              <Link href="/orders">
                <Button className="w-full bg-foreground hover:bg-foreground/90 text-background font-medium rounded-full h-12 transition-all active:scale-95">
                  Go to My Orders
                </Button>
              </Link>
              <Button
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('reviewOrder')}</p>
              </div>
              <button
                onClick={closeCheckout}
                className="p-2 hover:bg-accent rounded-full transition-all active:scale-95"
            ...
            <form className="space-y-4" onSubmit={handleCheckoutSubmit}>
              {savedAddresses.length > 0 && (
                <div className="space-y-1">
                  <label className="text-sm font-medium text-foreground">{t('select_saved_address')}</label>
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-foreground">{t('deliveryNotes')}</label>
                <Textarea
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3 rounded-2xl border border-border px-4 py-3 bg-background">
                  <input type="radio" id="cod" name="payment" checked readOnly />
                  <label htmlFor="cod" className="text-sm font-medium text-foreground">
                    {t('cashOnDelivery')}
                  </label>
                </div>
              </div>
