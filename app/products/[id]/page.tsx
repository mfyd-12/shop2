  ...
  const handleToggleFavorite = () => {
    ...
    toast({
      title: isCurrentlyFavorite 
        ? (language === 'ar' ? 'تم إزالة من المفضلة' : 'Removed from favorites')
        : t('addedToFavorites'),
      description: isCurrentlyFavorite 
        ? (language === 'ar' ? `تم إزالة ${productName} من المفضلة` : `${productName} removed from favorites`)
        : (language === 'ar' ? `تم إضافة ${productName} إلى المفضلة` : `${productName} added to favorites`),
      duration: 2000,
    })
  }
      ...
      <section className="px-4 pb-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="font-serif text-2xl font-semibold text-[#2A2723] inline-flex items-baseline gap-1">
              {language === 'ar' ? (
                <>
        ...
        >
          {language === 'ar' ? (
            <>
              أضف للسلة - <span className="inline-flex items-baseline gap-1"><span>{product.price * quantity}</span> <span>ر.س</span></span>
            </>
          ) : (
            <>
              Add to Cart - <span className="inline-flex items-baseline gap-1"><span>$</span> <span>{product.price * quantity}</span></span>
            </>
          )}
        </Button>
      </section>
